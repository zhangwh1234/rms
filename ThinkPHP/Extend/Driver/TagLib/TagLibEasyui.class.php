...
/**
 * easyUI标签库
 * 2015-05-02引入自wangdong
 */
defined('THINK_PATH') or exit();
class TagLibEasyui extends TagLib{
	// 标签定义
	 ...
	 * @return string|void
	 */
	public function _datagrid($attr,$tag) {
		$tag        = $this->parseXmlAttr($attr,'datagrid');
		if(strtolower(ACTION_NAME) == 'listview'){
			$action_name = 'index';
		}else{
			$action_name = ACTION_NAME;
		}
		$id    = !empty($tag['id']) ? $tag['id'] : strtolower(MODULE_NAME.'_'.$action_name.'_datagrid');
		...
		$dataOptions = array(
			...
			'striped'      => true,
			'pagination'   => true,
			'pageList'     => array(10,20,30,50,80,100),
			'pageSize'     => 10, //cookie('pagesize') ? cookie('pagesize') : C('DATAGRID_PAGE_SIZE'),
		);
