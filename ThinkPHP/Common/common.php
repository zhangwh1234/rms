 ...
 * @return Model
 */
function D($name='',$layer='',$domain='') {

	//取得表的连接信息
	import('COM.Db.SystemDb');
	$systemDb = new SystemDb();
	if(!empty($domain)){
		$connectConfig = $systemDb->getDbConnectionCli($name,$domain);  //Cli方式，必须自己带domain
	}else{
		$connectConfig = $systemDb->getDbConnection($name);
	}
	$db_type = 'mysql';
	$db_user = $connectConfig ['db_user'];
	$db_pwd = $connectConfig ['db_pwd'];
	$db_host = $connectConfig ['db_host'];
	$db_port = $connectConfig ['db_port'];
	$db_name = $connectConfig ['db_name'];
	$connectStr = "mysql://$db_user:$db_pwd@$db_host:$db_port/$db_name";
	//var_dump($connectStr);
    ...
    if(class_exists($class)) {
        $model      =   new $class(basename($name),'',$connectStr);
    }else {
        $model      =   new Model(basename($name),'',$connectStr);
    }  
    //分表操作
    if(!empty($connectConfig['trueTableName'])){
    	$model->setTableName($connectConfig['trueTableName']);
    }
    $_model[$name]  =  $model;
