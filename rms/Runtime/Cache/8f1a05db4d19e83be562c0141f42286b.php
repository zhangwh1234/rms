<?php if (!defined('THINK_PATH')) exit();?><div class="moduleMenu">
    <ul>
        ...
        <li style="margin-left: 20px;"><a href="javascript:;" onclick="NodeCreateviewModule.insert();"><img
                src=".__PUBLIC__/Images/newBtn.png" alt="" title="" border="0"></a></li>
        <li><a href="javascript:void(0);" onclick="NodeCreateviewModule.insert();">保存<span>^9</span></a></li>
...
<div class="moduleoperator">
    <form id="<?php echo ($moduleName); ?>CreateviewForm" name="<?php echo ($moduleName); ?>CreateViewForm" method="post"
...
<?php if(($uitype == 0)): ?><td width=20% class="dvtCellLabel" align=right><font color="red"><?php echo ($mandatory_field); ?></font><?php echo ($label); ?></td>
    ...
    <td width="100%" class="dvtCellLabel" align=center colspan="4">
        <table id="telAddressTable" width="90%" border=0 style="border: 1px solid #e0dddd; margin-top: 2px;"
            ...
            <?php if(empty($teladdress)): $__FOR_START_922864286__=1;$__FOR_END_922864286__=4;for($key=$__FOR_START_922864286__;$key < $__FOR_END_922864286__;$key+=1){ ?><tr style="height:25px;border: 1px solid black;background: #F0F0F0;" class="CaseRow">
...
<table id="<?php echo ($moduleName); echo (ucfirst(ACTION_NAME)); ?>productsTable" style="BORDER-COLLAPSE: collapse"  borderColor="#CCCCCC" cellSpacing="0" width="100%"
    ...
    <?php if(empty($orderproducts)): $__FOR_START_1265785005__=0;$__FOR_END_1265785005__=3;for($key=$__FOR_START_1265785005__;$key < $__FOR_END_1265785005__;$key+=1){ ?><tr>
...
<table id="orderPrintHandleTable" style="BORDER-COLLAPSE: collapse" borderColor="#CCCCCC" cellSpacing="0" width="100%"
    ...
    <?php if(empty($orderPrintHandle)): $__FOR_START_255575354__=0;$__FOR_END_255575354__=30;for($key=$__FOR_START_255575354__;$key < $__FOR_END_255575354__;$key+=1){ ?><tr>
                <td width="5%" align="center"><?php echo ($key+1); ?></td>
                            ...
                            <tr>
                                <td colspan="4" align="center">
                                    <a href="#" class="easyui-linkbutton" data-options="iconCls:'icons-other-tick'"
                                       onclick="NodeCreateviewModule.insert();"
...
<script>
    var NodeCreateviewModule = {
        ...
        //保存记录
        insert: function () {
            $('#NodeCreateviewForm').form('submit', {
                url: '__URL__/insert',
                onSubmit: function () {
                    //进行表单验证
                    if($('#NodecreateViewForm input[name=name]').val() == ''){
                        alert('模块名称不能为空!');
                        return false;
                    }
                    if($('#NodecreateViewForm input[name=title]').val() == ''){
                        alert('模块描述不能为空!');
                        return false;
                    }
                    var isValid = $(this).form('validate');
    ...
    $(function () {
        $('#NodeCreateviewForm input[name=name]').focus();
        NodeCreateviewModule.init();
    })
</script>