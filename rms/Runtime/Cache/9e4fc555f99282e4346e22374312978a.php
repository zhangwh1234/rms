...
<script type="text/javascript">
    var YingshouDoorBillListviewModule = {
        ...
        operate: function (val, rowData, rowIndex) {
            var btn = [];
            btn.push('<a href="javascript:void(0);" onclick="YingshouDoorBillListviewModule.detailview(' + rowData.doorbillid + ',\'' + rowData.date + '\')">查看</a>');
            if (rowData.noupdate == 1) {

            } else {
                btn.push('<a href="javascript:void(0);" onclick="YingshouDoorBillListviewModule.editview(' + rowData.doorbillid + ',\'' + rowData.date + '\')">编辑</a>');
        ...
        detailview: function (id, getDate) {
            var url = "<?php echo U('YingshouDoorBill/detailview');?>";
        ...
        editview: function (id, getDate) {
            var url = "<?php echo U('YingshouDoorBill/editview');?>";
            url += url.indexOf('?') != -1 ? '&record=' + id : '?id=' + id;
            url += '&getDate=' + getDate;
            IndexIndexModule.updateOperateTab(url);
        },
        ...
        deleteRecord: function (id, getDate) {
            ...
            $.messager.confirm('提示信息', '确定要删除吗？', function (result) {
                ...
                $.post("<?php echo U('YingshouDoorBill/delete');?>", {
                    record: id,
                    getDate: getDate
                }, function (res) {
