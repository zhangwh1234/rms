...
<input id="popupviewproductstable_row" value='<?php echo ($row); ?>' type="hidden" />
...
<script>
    ...
    $(function () {
        ...
        $('body').on('click', '.city-list p', function () {
            var data = $(this).text();
            saveHistory(data);
            rowNum = $('#popupviewproductstable_row').val();

            code = findCode[data];
            $("#YingshouRoomServiceAccountsCode_" + rowNum).val(code);
            $("#YingshouRoomServiceAccountsName_" + rowNum).val(data); //跳转到下一行
            ...
            $("#YingshouRoomServiceAccountsMoney_" + rowNum).val('33');
            BatchPaymentEditviewModule.sumAccountsMoney(rowNum);
