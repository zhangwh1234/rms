...
<script>
    var DiningReturnListviewModule = {
        ...
        setPagination:function(){
            //定义订单分页表
            var pager = $('#diningreturn_index_datagrid').datagrid().datagrid('getPager')
            pager.pagination({
                showRefresh: false,
        ...
        operate: function (val, rowData, rowIndex) {
            var btn = [];
            btn.push('<a href="javascript:void(0);" onclick="DiningReturnListviewModule.detailview(' + rowData.diningreturnid + ')">查看</a>');
            btn.push('<a href="javascript:void(0);" onclick="DiningReturnListviewModule.deleteRecord(' + rowData.diningreturnid + ')">删除</a>');
            return btn.join(' | ');
        },
        ...
        detailview: function (id) {
            var url = "<?php echo U('DiningReturn/detailview');?>";
        ...
        editview: function (id) {
            var url = "<?php echo U('DiningReturn/editview');?>";
            url += url.indexOf('?') != -1 ? '&record=' + id : '?id=' + id;
            IndexIndexModule.updateOperateTab(url);
        },
        ...
        deleteRecord: function (id) {
            ...
            $.messager.confirm('提示信息', '确定要删除吗？', function (result) {
                ...
                $.get("<?php echo U('DiningReturn/delete');?>", {record: id}, function (res) {
    ...
    quickKeyboardAction:function(){
        ...
        Mousetrap.bind(['ctrl+1','ctrl+f1','f1'], function(e) {
            ...
            if (tabOptions.title == '群发消息' && ($('#DiningReturnAction').val() == 'Listview')) {
    ...
    $(function(){
        DiningReturnListviewModule.init();
        setTimeout(function(){
            DiningReturnListviewModule.setPagination();
        },200);
    })
</script>