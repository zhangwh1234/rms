<?php if (!defined('THINK_PATH')) exit();?><div class="moduleMenu">
    <ul>
        ...
        <li><a href="javascript:void(0);" class="moduleName" >&nbsp;&gt;<?php echo (L("$moduleName")); ?></a></li>
        <li>&nbsp;&gt;查看操作</li>
    ...
    <?php if(is_array($orderactivity)): foreach($orderactivity as $key=>$vo): ?><tr>
        ...
        <td width="45%" align=left >
            <span style="background: #F5F5F5;"><?php echo ($vo["money"]); ?></span>
                        ...
                        <?php if(!empty($orderpayment)): ?><table border="0" cellspacing="0" cellpadding="0" width="100%"
...
<table id="DetailOrderFormPayment" style="BORDER-COLLAPSE: collapse"  borderColor="white" cellSpacing="0" width="100%"
       align="center" border="1">
    <?php if(is_array($orderpayment)): foreach($orderpayment as $key=>$vo): ?><tr>
        <td width="10%" align="right" >
            <span style="margin-right: 5px;">支付名称:</span></td>
        <td width="45%"  align="left" style="background: #F5F5F5;">
            <span><?php echo ($vo["name"]); ?></span>
        </td>
        <td width="10%" align="right" >
            <span style="margin-right:5px;">支付金额:</span></td>
        <td  align="left" style="background: #F5F5F5;">
            <span><?php echo ($vo["money"]); ?></span>
        </td>
    </tr><?php endforeach; endif; ?>
...
<input id="OrderHandleAction" type="hidden" value="Detailview"/>

<script>
    var OrderHandleDetailviewModule = {
        //初始化
        init:function(){
    ...
    $(function(){
        OrderHandleDetailviewModule.init();
    })
</script>