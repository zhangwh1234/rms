...
class OrderDistributionAction extends ModuleAction
{
    ...
    public function searchviewCompany()
    {
        ...
        if (IS_POST) {
            ...
            $this->assign('companyName', $searchCompany);
            ...
            $companymgrModel = D('companymgr');
            $where = array();
            $where['name'] = $searchCompany;
            $where['domain'] = $this->getDomain();
            $companymgrResult = $companymgrModel->where($where)->find();
            $this->assign('cancelenable', $companymgrResult['cancelenable']);
     ...
     * 未派单的查询
     */
    public function searchviewCompanyNoPaidan()
    {
        ...
        if (IS_POST) {
            ...
            $url = U('OrderDistribution/searchviewCompanyNoPaidan', array('searchTextCompany' => $searchCompany));
            $this->assign('url', $url);
            // 加入模块id到listHeader中
            // array_unshift($listFields,$moduleNameId);
            $listHeader = $listFields;
