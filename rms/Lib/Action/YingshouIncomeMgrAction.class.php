...
class YingshouIncomeMgrAction extends YingshouAction
{
    ...
    public function listviewTwo()
    {
        if (IS_POST) {
            ...
            $listResult = $Model->where($where)->field($selectFields)->limit($Page->firstRow . ',' . $Page->listRows)->order("date desc,shenhe asc")->select(); //lastdatetime desc,
    ...
    public function getAccountsByCode()
    {
        ...
        $where['company'] = $company;
        $where['domain'] = $this->getDomain();
        $accountsbillsmingxiResult = $accountsbillsmingxiModel->where($where)->limit(100)->order('date desc')->select();
    ...
    public function popupAccountsview()
    {
        ...
        $this->display('YingshouIncomeMgr/selectpaymentmgrview');
        return;
        if (IS_POST) {
            // 取得模块的名称
            $moduleName = $this->getActionName();
    ...
    public function save_slave_table($record, $getDate)
    {
        ...
        $data['type'] = $_REQUEST['operation'];
        $data['company'] = $company;
        $data['domain'] = $this->getDomain();
        $data['create_time'] = date('Y-m-d H:i:s');
        $accountsbillsmingxiModel = M('accountsbillsmingxi', " ", $connectionDb);
    ...
    public function delete_slave_table($record, $getDate)
    {
        ...
        //$where['domain'] = $this->getDomain();
        $accountsbillsmingxiResult = $accountsbillsmingxiModel->where($where)->delete();
    }
    ...
    public function audit()
    {
        ...
        //连接字符串
        $connectionDb = $this->connectReveueDb($startDate);
        $reveueConnectDb = $connectionDb; //防止出错，重复
        //连接的数据表
        $tableName = $focus->getTableName();
        ...
        $where['shenhe'] = 0;
        $where['domain'] = $domain;
        $incomemgrResult = $Model->where($where)->select();
        ...
        foreach ($incomemgrResult as $key => $value) {
            //删除原来的数据
            $where = array();
            $where['ordersn'] = $value['incomemgrid'];
            ...
            //保存数据到客户明细表中
            $data = array();
            $data['ordersn'] = $value['incomemgrid'];
            $data['code'] = $value['code'];
            $data['name'] = $value['name'];
            $data['type'] = $value['operation'];
            if ($data['type'] == '还欠') {
        ...
        if ($result) {
            ...
            $info['status'] = 0;
            $info['info'] = '审核失败' . $Model->getLastSql();
            $this->ajaxReturn(json_encode($info), 'EVAL');
        }
    ...
    public function recover()
    {
        ...
        foreach ($incomemgrResult as $ky => $value) {
            $where = array();
            $where['ordersn'] = $value['incomemgrid'];
            $where['company'] = $value['company'];
            $where['date'] = $value['date'];
    ...
    public function getCompanyPayment()
    {
        ...
        $where['domain'] = $this->getDomain();
        $where['is_shenhe'] = 1; //只有经过审核的，才能使用
